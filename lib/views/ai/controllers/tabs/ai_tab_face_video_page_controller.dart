...
class _DataKeeper
  ...
  Future<List<AiStencilModel>?> dataFetcher(int page,
    ...
    return Api.fetchAiFaceVideoStencil(
      page: page,
      pageSize: _pageSize,
...
class AiTabFaceVideoPageController extends BaseRefreshDefaultTabController
  ...
  void onTapMake(AiStencilModel stencil, AiBytesImage? file) async {
    ...
    if (costType == AiCostType.fail) {
      ...
      AiCostGoldConfirmDialog.faceVideo(
        stencil.amount,
  ...
  void _onMakeConfirm(AiStencilModel stencil, AiBytesImage file) async {
    final future = Api.createAiFaceVideoOrder(stencil.stencilId, file.bytes);
  ...
  Future<void> fetchClassify() async {
    ...
    final classifyList = await Api.fetchAiFaceVideoClassify();
